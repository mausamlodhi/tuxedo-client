...
// Types
interface AddSuspenderFormValues {
  style: string;
  color: string;
  description: string;
  images: File[];
...
interface AddSuspendersFormProps {
  isOpen?: boolean;
  onClose?: () => void;
  onSubmit?: (values: AddSuspenderFormValues) => void;
  type?: string;
...
// Validation Schema
const addSuspendersValidationSchema = {
  style: (value: string): string | undefined => {
    if (!value) return "Suspenders style is required";

    if (value.length < 3)
      return "Suspenders Style must be at least 3 characters";
    if (value.length > 16)
      return "Suspenders Style must not exceed 16 characters";
    return undefined;
  },
...
const AddSuspendersForm: React.FC<AddSuspendersFormProps> = ({
  isOpen = true,
  onClose,
  onSubmit,
  type,
  ...
  const validate = (
    values: AddSuspenderFormValues
  ): Partial<AddSuspenderFormValues> => {
    const errors: Partial<AddSuspenderFormValues> = {};

    (
      Object.keys(addSuspendersValidationSchema) as Array<
        keyof AddSuspenderFormValues
      >
    ).forEach((field) => {
      const validator = addSuspendersValidationSchema[
        field as keyof typeof addSuspendersValidationSchema
  ...
  const handleSubmit = (
    values: AddSuspenderFormValues,
    { setSubmitting, resetForm }: FormikHelpers<AddSuspenderFormValues>
  ): void => {
    ...
    if (onSubmit) {
      ...
      alert("Suspenders added successfully!");
    }
    setSubmitting(false);
    resetForm();
  };
    ...
    ${theme ? "bg-[#FFFFFF] text-[#2D333C]" : "bg-[#2D333C] text-[#FFFFFF]"}`}
      >
        ...
        >
          ...
          >
            {type === "add" ? "Add Suspenders" : "Update Suspenders"}
          </h2>
          <button
            onClick={onClose}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <Field
                      as="input"
                      name="style"
                      placeholder="Enter suspenders style"
                      rows={4}
                      ref={(el: any) => (fieldRefs.current.style = el)}
...
export default AddSuspendersForm;
