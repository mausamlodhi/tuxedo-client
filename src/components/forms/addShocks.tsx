...
interface AddShocksFormProps {
  isOpen?: boolean;
  onClose?: () => void;
  onSubmit?: (values: AddShocksFormValues) => void;
  type?: string;
...
const addTieValidationSchema = {
  tieStyle: (value: string): string | undefined => {
    if (!value) return "Socks style is required";

    if (value.length < 3) return "Socks Style must be at least 3 characters";
    if (value.length > 16) return "Socks Style must not exceed 16 characters";
    return undefined;
  },
...
const AddShocksForm: React.FC<AddShocksFormProps> = ({
  isOpen = true,
  onClose,
  onSubmit,
  type,
  ...
  const validate = (
    values: AddShocksFormValues
  ): Partial<AddShocksFormValues> => {
    const errors: Partial<AddShocksFormValues> = {};

    (
      Object.keys(addTieValidationSchema) as Array<keyof AddShocksFormValues>
    ).forEach((field) => {
  ...
  const handleSubmit = (
    values: AddShocksFormValues,
    { setSubmitting, resetForm }: FormikHelpers<AddShocksFormValues>
  ): void => {
    ...
    ${theme ? "bg-[#FFFFFF] text-[#2D333C]" : "bg-[#2D333C] text-[#FFFFFF]"}`}
      >
        ...
        >
          ...
          >
            {type === "add" ? "Add Socks" : "Update Socks"}
          </h2>
          <button
            onClick={onClose}
