...
interface AddShirtFormProps {
  ...
  onClose?: () => void;
  onSubmit?: (values: AddShirtFormValues) => void;
  vests: VestInterface[];
...
// Validation Schema
const addShirtValidationSchema = {
  style: (value: string): string | undefined => {
    if (!value) return "Shirt style is required";
    if (value.length < 3) return "Shirt style must be at least 3 characters";
    if (value.length > 16) return "Shirt style must not exceed 16 characters";
    return undefined;
  },
  colorId: (value: string): string | undefined => {
    if (!value) return "Shirt Color is required";
    return undefined;
  },
  // matching_vest: (value: string): string | undefined => {
  //   if (!value) return 'Shirt Matching vest selection is required';
  //   return undefined;
  // },
  ...
  description: (value: string): string | undefined => {
    if (!value) return "Shirt Description is required";
    if (value.length < 10)
      return "Shirt Description must be at least 10 characters";
...
const AddShirtForm: React.FC<AddShirtFormProps> = ({
  isOpen = true,
  onClose,
  onSubmit,
  type,
  ...
  const initialValues = {
    ...
    // matching_vest: selectedData?.vest?.id,
    description: selectedData?.description,
    images: selectedData?.images || [],
    buy_price: selectedData?.buy_price || null,
  ...
  const validate = (
    values: AddShirtFormValues
  ): Partial<AddShirtFormValues> => {
    const errors: Partial<AddShirtFormValues> = {};

    (
      Object.keys(addShirtValidationSchema) as Array<keyof AddShirtFormValues>
    ).forEach((field) => {
      const validator = addShirtValidationSchema[
        field as keyof typeof addShirtValidationSchema
  ...
  const handleSubmit = (
    values: AddShirtFormValues,
    { setSubmitting, resetForm }: FormikHelpers<AddShirtFormValues>
  ): void => {
    if (!values.buy_price) {
    ...
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      ...
      >
        ...
        >
          ...
          >
            {type === "add" ? "Add Shirt" : "Update Shirt"}
          </h2>
          <button
            onClick={onClose}
