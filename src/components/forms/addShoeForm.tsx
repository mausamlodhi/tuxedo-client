...
interface AddShoesFormValues {
  ...
  collection: string;
  detail: string;
  matching_socks: number;
}

interface AddShoesFormProps {
  isOpen?: boolean;
  onClose?: () => void;
  onSubmit?: (values: AddShoesFormValues) => void;
  type?: string;
  ...
  matching_socks: SocksInterface[];
}

// Validation Schema
const addShoesValidationSchema = {
  shoeStyle: (value: string): string | undefined => {
    if (!value) return "Shoe style is required";
    if (value.length < 3) return "Shoes Style must be at least 3 characters";
    if (value.length > 16) return "Shoes Style must not exceed 16 characters";
    return undefined;
  },
  color: (value: string): string | undefined => {
    if (!value) return "Color is required";
    if (value.length < 3) return "Color must be at least 3 characters";
    if (value.length > 8) return "Color must not exceed 8 characters";
    return undefined;
  },
  ...
  matching_socks: (value: number | string): string | undefined => {
    if (!value) return "Matching Socks is required";
    return undefined;
  }
};

const AddShoesForm: React.FC<AddShoesFormProps> = ({
  isOpen = true,
  onClose,
  onSubmit,
  type,
  ...
  const initialValues = {
    ...
    matching_socks: selectedData?.id ? selectedData?.socks?.id || "#" : 0,
  };
  ...
  const validate = (
    values: AddShoesFormValues
  ): Partial<AddShoesFormValues> => {
    const errors: Partial<AddShoesFormValues> = {};

    (
      Object.keys(addShoesValidationSchema) as Array<keyof AddShoesFormValues>
    ).forEach((field) => {
      const validator = addShoesValidationSchema[
        field as keyof typeof addShoesValidationSchema
  ...
  const handleSubmit = (
    values: AddShoesFormValues,
    { setSubmitting, resetForm }: FormikHelpers<AddShoesFormValues>
  ): void => {
    if (!values.buy_price) values.buy_price = null;
    if (onSubmit) {
      onSubmit(values);
    }
    setSubmitting(false);
              ...
              <Form className="flex flex-col max-h-[73vh]">
                ...
                >
                  <div className=" grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <Field
                        ...
                        rows={4}
                        innerRef={(el: any) =>
                          (fieldRefs.current.shoeStyle = el)
                        }
                        className={`w-full px-3 py-2 bg-[#313A46] border rounded-lg  focus:outline-none ${errors.shoeStyle && touched.shoeStyle
                            ? "border-red-500"
                            : "border-slate-600"
                          } ${theme
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div
                      ref={(el: any) => (fieldRefs.current.matching_socks = el)}
                    >
                      <label
                        className={`block text-sm font-medium mb-2 ${theme ? "text-[#2D333C]" : "text-[#FFFFFF]"
                          }`}
                      ...
                      <SelectField
                        name="matching_socks"
                        options={(matching_socks || []).map((socks) => ({
                          value: socks.id,
                          label: `${socks.style} - ${socks.description}`,
                        }))}
                        placeholder="Select Matching Socks"
                        bgColor={theme ? "#ffffff" : "#313A46"}
                        textColor={theme ? "#2D333C" : "#ffffff"}
                        borderColor={theme ? "#CBD5E1" : "#475569"}
                        menuBgColor={theme ? "#ffffff" : "#313A46"}
                  ...
                  <div>
                    ...
                    <Field
                      ...
                      className={`w-full px-3 py-2 bg-[#313A46] border rounded-lg  placeholder-gray-400 focus:outline-none ${errors.description && touched.description
                          ? "border-red-500"
                          : "border-slate-600"
                        } ${theme
                          ? "bg-white text-black-700"
                ...
                >
                  ...
                  {isSubmitting ? (
                    ...
                    <button
                      ...
                      disabled={isSubmitting}
                      className={` px-6 py-2 w-35 rounded-sm h-10 font-medium transition-colors cursor-pointer  ${theme
