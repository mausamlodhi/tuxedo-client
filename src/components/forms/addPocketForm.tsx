...
// Types
interface AddPocketSquareFormValues {
  pocketStyle: string;
  description: string;
  images: File[];
  buy_price?: number;
...
interface AddPocketSquareFormProps {
  isOpen?: boolean;
  onClose?: () => void;
  onSubmit?: (values: AddPocketSquareFormValues) => void;
...
// Options
const pocketStyleOptions: string[] = [
  "Single Point",
  "Double Point",
  "Flat",
  "Winged",
...
// Validation Schema
const addPocketSquareValidationSchema = {
  pocketStyle: (value: string): string | undefined => {
    if (!value) return "Pocket style is required";
    if (value.length < 3)
      return "Pocket Square Style must be at least 3 characters";
    if (value.length > 16)
      return "Pocket Square Style must not exceed 16 characters";
    return undefined;
  },
...
const AddPocketSquareForm: React.FC<AddPocketSquareFormProps> = ({
  isOpen = true,
  onClose,
  onSubmit,
  theme,
  ...
  const initialValues = {
    pocketStyle: selectedData?.style || "",
  ...
  const removeImage = (
    ...
    values: AddPocketSquareFormValues
  ): void => {
  ...
  const validate = (
    values: AddPocketSquareFormValues
  ): Partial<AddPocketSquareFormValues> => {
    const errors: Partial<AddPocketSquareFormValues> = {};

    (
      Object.keys(addPocketSquareValidationSchema) as Array<
        keyof AddPocketSquareFormValues
      >
    ).forEach((field) => {
      const validator = addPocketSquareValidationSchema[
        field as keyof typeof addPocketSquareValidationSchema
  ...
  const handleSubmit = (
    values: AddPocketSquareFormValues,
    { setSubmitting, resetForm }: FormikHelpers<AddPocketSquareFormValues>
  ): void => {
    ...
    if (onSubmit) {
      ...
      alert("Pocket Square added successfully!");
    }
    ...
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      ...
      >
        ...
        >
          ...
          >
            {type === "edit" ? "Update Pocket-Square" : "Add Pocket-Square"}
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6">
          <Formik<AddPocketSquareFormValues>
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <Field
                        ...
                        className={`w-full px-3 no-arrows py-2 border rounded-lg focus:outline-none ${
                  ...
                  <div>
                    ...
                    <Field
                      ...
                      className={`w-full px-3 py-2  border rounded-lg  placeholder-gray-400 focus:outline-none ${
...
export default AddPocketSquareForm;
